...
std::vector<float> convertStringVectortoFloatVector(
...
bool parseLine(const double[] &data_array, geometry_msgs::Vector3 &position, 
								geometry_msgs::Vector3 &linearVelocity, geometry_msgs::Vector3 &linearAcceleration, 
  ...
  if (result) {
    ...
    for (std::string& line : vecOfStr) {
      ...
      geometry_msgs::Vector3 position;
		  ...
		  std_msgs::UInt64 timestampNanoseconds;
      parseLine(data_array,position,linearVelocity,linearAcceleration,timestampNanoseconds);
...
int main(int argc, char** argv) {
  ...
  // setup ...
  ros::Publisher pub =
      nh.advertise<Dimos_package::FullStateTrajectory>("pen_trajectory", 10);

  // do shit
  std::vector<Dimos_package::FullStateStamped> vector_of_states =
      calculatesAllStates();
